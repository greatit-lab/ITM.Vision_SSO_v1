...
<template>
  ...
  >
    ...
    <div
      class="mb-4 bg-white dark:bg-[#111111] p-1.5 rounded-xl border border-slate-200 dark:border-zinc-800 flex flex-wrap gap-2 items-center justify-between shadow-sm shrink-0 transition-colors duration-300"
    >
      <div class="flex flex-wrap items-center flex-1 gap-2 px-1 py-1">
        ...
        <div v-if="filter.status" class="flex items-center gap-2 px-3 py-1 ml-2 text-xs font-bold rounded-full bg-slate-100 dark:bg-zinc-800 text-slate-600 dark:text-slate-300 animate-fade-in">
            <i class="pi pi-filter text-[10px]"></i>
            Filtered by: {{ filter.status }}
            <button @click="setStatusFilter(null)" class="ml-1 hover:text-rose-500"><i class="pi pi-times"></i></button>
        </div>
      </div>
      <div
    ...
    <div v-if="hasSearched" class="grid grid-cols-1 gap-4 mb-4 md:grid-cols-4 shrink-0 animate-fade-in">
      <div 
        ...
        :class="{'ring-2 ring-slate-400 ring-offset-2 dark:ring-offset-zinc-950': filter.status === null}"
      >
      ...
      <div 
        ...
        :class="{'ring-2 ring-rose-500 ring-offset-2 dark:ring-offset-zinc-950': filter.status === 'Critical'}"
      >
      ...
      <div 
        ...
        :class="{'ring-2 ring-amber-500 ring-offset-2 dark:ring-offset-zinc-950': filter.status === 'Warning'}"
      >
      ...
      <div 
        ...
        :class="{'ring-2 ring-emerald-500 ring-offset-2 dark:ring-offset-zinc-950': filter.status === 'Good'}"
      >
    ...
    <div v-if="hasSearched" class="flex flex-col h-[650px] shrink-0 lg:flex-row gap-4 overflow-hidden animate-fade-in">
      ...
      <div class="flex flex-col w-full lg:w-1/3 overflow-hidden bg-white border shadow-sm dark:bg-zinc-900 border-slate-200 dark:border-zinc-800 rounded-xl">
        <div class="flex items-center justify-between px-4 py-3 border-b border-slate-100 dark:border-zinc-800">
            ...
            <span class="text-[10px] text-slate-400">*Global Ranking (Top 10)</span>
        </div>
        <div class="relative flex-1 w-full min-h-0">
            <EChart v-if="chartData.length > 0" :option="chartOption" />
            <div v-else class="flex items-center justify-center h-full text-xs text-slate-400">No data available</div>
        </div>
      </div>
      ...
      <div class="flex flex-col flex-1 w-full lg:w-2/3 overflow-hidden bg-white border shadow-sm dark:bg-zinc-900 border-slate-200 dark:border-zinc-800 rounded-xl">
        ...
        <div class="relative w-full flex-1 min-h-0">
            ...
            >
                ...
                <Column field="eqpId" header="EQP ID" sortable style="min-width: 100px">
                    <template #body="{ data }">
                         <span class="font-bold text-slate-700 dark:text-slate-200">{{ data.eqpId }}</span>
                    </template>
                </Column>

                <Column field="lampId" header="Lamp ID" sortable style="min-width: 100px">
                    <template #body="{ data }">
                         <span class="font-mono text-slate-700 dark:text-slate-200">{{ data.lampId }}</span>
                    </template>
                </Column>
                
                <Column field="lastChanged" header="Last Changed" sortable style="min-width: 120px">
                    <template #body="{ data }">
                        <span class="font-mono text-slate-500">{{ data.lastChanged || '-' }}</span>
                    </template>
                </Column>

                <Column field="usageRatio" header="Life Usage" sortable style="min-width: 200px">
                    <template #body="{ data }">
                ...
                <Column field="status" header="Status" sortable style="min-width: 100px" alignFrozen="right" frozen>
                    <template #body="{ data }">
                        <span 
    ...
    return {
        ...
        tooltip: {
            ...
            formatter: (params: any) => {
                ...
                return `
                    <div class="font-bold mb-1">${item.eqpId} <span style="font-weight:normal; opacity:0.7">(${item.lampId})</span></div>
                    <div class="text-xs">Age: ${item.ageHour} / ${item.lifespanHour} hrs</div>
                    <div class="text-xs font-bold mt-1">Ratio: ${item.usageRatio.toFixed(2)}%</div>
                `;
            }
        },
        grid: { 
...
const getStatusBadgeClass = (status: string) => {
    switch (status) {
        case 'Critical': return 'bg-rose-50 text-rose-600 border-rose-200 dark:bg-rose-900/20 dark:text-rose-400 dark:border-rose-900/30';
        case 'Warning': return 'bg-amber-50 text-amber-600 border-amber-200 dark:bg-amber-900/20 dark:text-amber-400 dark:border-amber-900/30';
        case 'Good': return 'bg-emerald-50 text-emerald-600 border-emerald-200 dark:bg-emerald-900/20 dark:text-emerald-400 dark:border-emerald-900/30';
