                      ...
                      <Column header="Action" align="center" style="width: 100px; min-width: 100px;">
                        <template #body="{ data }">
                          <div class="flex items-center justify-center gap-2 flex-nowrap w-full">
                            ...
                            <Button v-if="isAdmin" icon="pi pi-trash" text rounded severity="danger" size="small" class="!w-6 !h-6" @click="removeSdwt(data.id)" />
                          </div>
                        </template>
                      </Column>
                    </DataTable>
             ...
             <div class="flex flex-row h-full gap-4 p-4 min-h-0">
               <div class="flex flex-col flex-1 h-full min-h-0 overflow-hidden bg-white border rounded-lg shadow-sm border-slate-200 dark:border-zinc-800">
                  <div class="flex items-center justify-between p-3 border-b shrink-0 border-slate-100 dark:border-zinc-800 bg-slate-50/50 dark:bg-zinc-900">
                    ...
                    <Button v-if="isManagerOrAdmin" label="추가" icon="pi pi-plus" size="small" class="!py-1 !text-xs" @click="openSeverityDialog()" />
                  </div>
                    ...
                    <div class="absolute inset-0">
                      <DataTable :value="severities" scrollable scrollHeight="100%" class="h-full text-xs p-datatable-sm" stripedRows :loading="loading" sortField="errorId" :sortOrder="1">
                          ...
                          <Column header="Action" align="center" style="width: 20%; min-width: 100px;">
                            <template #body="{ data }">
                              <div class="flex items-center justify-center gap-2 flex-nowrap w-full">
                                <Button v-if="isManagerOrAdmin" icon="pi pi-pencil" text rounded severity="info" size="small" class="!w-6 !h-6" @click="openSeverityDialog(data)" />
                                <Button v-if="isAdmin" icon="pi pi-trash" text rounded severity="danger" size="small" class="!w-6 !h-6" @click="removeSeverity(data.errorId)" />
                              </div>
                            </template>
                          </Column>
                      </DataTable>
               ...
               <div class="flex flex-col flex-1 h-full min-h-0 overflow-hidden bg-white border rounded-lg shadow-sm border-slate-200 dark:border-zinc-800">
                  <div class="flex items-center justify-between p-3 border-b shrink-0 border-slate-100 dark:border-zinc-800 bg-slate-50/50 dark:bg-zinc-900">
                    ...
                    <Button v-if="isManagerOrAdmin" label="추가" icon="pi pi-plus" size="small" class="!py-1 !text-xs" @click="openMetricDialog()" />
                  </div>
                  <div class="flex-1 min-h-0 overflow-hidden relative">
                    <div class="absolute inset-0">
                      <DataTable :value="metrics" scrollable scrollHeight="100%" class="h-full text-xs p-datatable-sm" stripedRows :loading="loading" sortMode="multiple" :multiSortMeta="metricsMultiSortMeta">
                        ...
                        <Column header="Action" align="center" style="width: 20%; min-width: 100px;">
                           <template #body="{ data }">
                             <div class="flex items-center justify-center gap-2 flex-nowrap w-full">
                               <Button v-if="isManagerOrAdmin" icon="pi pi-pencil" text rounded severity="info" size="small" class="!w-6 !h-6" @click="openMetricDialog(data)" />
                               <Button v-if="isAdmin" icon="pi pi-trash" text rounded severity="danger" size="small" class="!w-6 !h-6" @click="removeMetric(data.metricName)" />
                             </div>
                           </template>
                         </Column>
                      </DataTable>
...
const userRole = computed(() => String(authStore.user?.role || '').trim().toUpperCase());

const isAdmin = computed(() => userRole.value === 'ADMIN');
const isManagerOrAdmin = computed(() => userRole.value === 'ADMIN' || userRole.value === 'MANAGER');
...
const loadSdwts = async () => { 
  if (isManagerOrAdmin.value) {
    sdwts.value = (await InfraApi.getInfraSdwt()).data; 
  }
};
...
const refreshCurrentTab = () => {
  ...
  else if(activeTab.value === '1' && isManagerOrAdmin.value) loadSdwts().finally(() => loading.value = false);
...
watch(activeTab, (newVal) => {
  if (newVal === '1' && !isManagerOrAdmin.value) {
    activeTab.value = '0';
  }
});
...
onMounted(() => {
  ...
  if (isManagerOrAdmin.value) promises.push(loadSdwts());
