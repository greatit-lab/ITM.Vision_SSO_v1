...
<template>
  <div class="min-h-screen bg-slate-50 dark:bg-[#09090b] p-6 lg:p-10">
    <div class="max-w-4xl mx-auto">
      ...
      <div class="mb-8">
        <h1 class="text-2xl font-bold text-slate-900 dark:text-white">질문 작성</h1>
        <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">
          관리자에게 문의사항을 남기거나 시스템 이슈를 제보해 주세요.
        </p>
      </div>
      ...
      <div class="bg-white dark:bg-zinc-900 rounded-2xl p-6 shadow-sm border border-slate-200 dark:border-zinc-800">
        <form @submit.prevent="submitPost" class="space-y-6">
          ...
          <div class="grid gap-6 md:grid-cols-[1fr_200px]">
            <div class="space-y-1.5">
              ...
              <input 
                ...
                class="w-full px-4 py-2.5 bg-slate-50 dark:bg-zinc-800 border border-slate-200 dark:border-zinc-700 rounded-xl outline-none focus:border-indigo-500 transition-all font-medium text-slate-900 dark:text-white"
              >
            </div>
            ...
            <div class="space-y-1.5">
              ...
              <select 
                ...
                class="w-full px-4 py-2.5 bg-slate-50 dark:bg-zinc-800 border border-slate-200 dark:border-zinc-700 rounded-xl outline-none focus:border-indigo-500 transition-all font-medium text-slate-900 dark:text-white appearance-none cursor-pointer"
              >
          ...
          <div class="space-y-1.5">
            ...
            <div class="editor-wrapper bg-white dark:bg-zinc-800 rounded-xl border border-slate-200 dark:border-zinc-700 overflow-hidden">
              <QuillEditor 
                ...
                theme="snow"
                toolbar="essential"
                placeholder="상세 내용을 작성해 주세요... (스크린샷 이미지를 붙여넣을 수 있습니다)"
          ...
          <div class="flex items-center gap-2">
            ...
            >
            <label for="isSecret" class="text-sm font-medium text-slate-700 dark:text-slate-300 cursor-pointer select-none">
              <i class="pi pi-lock mr-1 text-xs"></i> 비밀글 설정 (작성자와 관리자만 볼 수 있습니다)
            </label>
          </div>

          <div class="pt-4 flex gap-3 justify-end border-t border-slate-100 dark:border-zinc-800">
            <button 
              type="button" 
              @click="$router.back()"
...
<style>
/* Quill Editor 스타일 오버라이드 (다크모드 지원) */
.ql-toolbar {
