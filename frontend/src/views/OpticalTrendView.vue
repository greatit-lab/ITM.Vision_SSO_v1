...
<template>
  ...
  >
    ...
    <div
      v-if="hasSearched"
      class="flex flex-col flex-1 gap-4 overflow-y-auto custom-scrollbar pr-1 pb-4 relative min-h-0 animate-fade-in"
    >
      <div
        v-if="isLoading"
        class="absolute inset-0 z-50 flex flex-col items-center justify-center bg-white/90 dark:bg-zinc-900/90 backdrop-blur-sm rounded-xl border border-transparent"
      >
        <div class="relative">
          <div
      ...
      <div
        v-else-if="!isLoading && hasSearched && trendData.length === 0"
      ...
      >
        ...
        >
          <i
            class="text-4xl pi pi-exclamation-circle text-slate-300 dark:text-zinc-600"
          ></i>
        </div>
        <p class="text-sm font-bold text-slate-500">No Data Found</p>
        <p class="text-xs">
          No optical trend data found for the selected condition.
        </p>
      </div>
    </div>
...
const isDependentLoading = ref(false);  
...
watch(
  ...
  ([newStart, newEnd], [oldStart, oldEnd]) => {
    ...
    if (filter.eqpId) {
      loadRecipeOptions(filter.eqpId).then(() => {
...
:deep(.date-picker .p-inputtext) {
  @apply !h-7 !text-[12px] !py-1 !pl-2 !pr-7 w-full; 
}
