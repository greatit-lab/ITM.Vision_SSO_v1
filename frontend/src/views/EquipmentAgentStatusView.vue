<!-- frontend/src/views/EquipmentAgentStatusView.vue -->
<template>
  <div class="flex flex-col h-full w-full font-sans transition-colors duration-500 bg-[#F8FAFC] dark:bg-[#09090B] overflow-hidden relative p-2 md:p-4">
    ...
    <div class="relative z-10 flex flex-col items-start justify-center gap-1 mb-4 px-1 shrink-0">
    ...
    <div class="relative z-10 flex-1 min-h-0 w-full overflow-hidden flex flex-col gap-3">
      ...
      <div v-else class="w-full min-h-0 flex flex-col gap-2 shrink-0 transition-all duration-300 ease-in-out"
           :style="{ maxHeight: selectedSelection ? '55%' : '100%' }">
        
        <div class="flex items-center justify-between px-1 shrink-0 mb-1">
          ...
          <div class="flex items-center gap-2">
            ...
            <button 
              ...
              :disabled="isLoading"
              title="매트릭스 데이터를 최신 상태로 새로고침합니다"
            >
              <i class="pi pi-refresh" :class="{'pi-spin': isLoading, 'text-emerald-500 dark:text-emerald-400': !isLoading}"></i> Refresh
            </button>
          </div>
        </div>

        <div class="w-full flex-1 overflow-auto custom-scrollbar bg-white border shadow-sm rounded-xl dark:bg-zinc-900 border-slate-200 dark:border-zinc-700">
          <table class="w-full text-left border-collapse min-w-max bg-white dark:bg-zinc-900">
            ...
            <tfoot class="sticky bottom-0 z-20">
              <tr>
                ...
                <td class="border border-slate-200 dark:border-zinc-700 bg-slate-100 dark:bg-zinc-800 p-3 text-sm font-extrabold text-emerald-700 dark:text-emerald-400 text-center shadow-[0_-1px_0_0_rgba(0,0,0,0.1)]">
                  {{ grandTotals['_all'] || 0 }}
                </td>
              </tr>
            </tfoot>
      ...
      <transition name="slide-up">
        <div v-if="selectedSelection" class="w-full flex-1 min-h-0 flex flex-col bg-white dark:bg-zinc-900 border border-emerald-400 shadow-md rounded-xl dark:border-emerald-600 overflow-hidden transition-all duration-300">
          ...
          <div class="flex-1 overflow-auto custom-scrollbar px-4 pb-4 pt-0 bg-white dark:bg-zinc-900">
            <div :class="rightDetailData.length > 0 ? 'grid grid-cols-1 xl:grid-cols-2 gap-x-6 gap-y-4 items-start pt-4' : 'w-full pt-4'">
              
              <table class="w-full text-left border-collapse bg-white dark:bg-zinc-900">
                <thead class="sticky top-0 z-10 bg-slate-100 dark:bg-zinc-800 shadow-sm">
                  <tr>
                    ...
                    <th class="border border-slate-200 dark:border-zinc-700 p-2 text-[11px] font-extrabold text-slate-500 dark:text-slate-400 text-center uppercase">Agent Version</th>
                ...
                <tbody>
                  <tr v-for="(item, idx) in leftDetailData" :key="item.eqpId" class="hover:bg-slate-50 dark:hover:bg-zinc-800/50">
                    ...
                    <td class="border border-slate-200 dark:border-zinc-700 p-2 text-xs text-center align-middle">
                      <div v-if="item.appVer && item.appVer === latestAgentVersion" class="inline-flex items-center justify-center px-2 py-0.5 font-extrabold text-[11px] rounded-full bg-indigo-50 text-indigo-600 border border-indigo-100 dark:bg-indigo-500/10 dark:text-indigo-400 dark:border-indigo-500/20 shadow-sm" title="최신 버전 에이전트 구동 중">
                        <span>{{ item.appVer }}</span>
                      </div>
                      <span v-else class="font-medium text-slate-400 dark:text-zinc-500 font-mono">{{ item.appVer || '-' }}</span>
                    </td>
                  ...
                  <tr v-if="detailData.length === 0">
                    <td colspan="6" class="border border-slate-200 dark:border-zinc-700 p-6 text-center text-xs text-slate-500 dark:text-slate-400">선택된 장비 정보가 없습니다.</td>
                  </tr>
                </tbody>
              </table>
...
const latestAgentVersion = computed(() => {
  if (!rawEquipmentList.value.length) return "";
  const versions = rawEquipmentList.value
    .map(eq => eq.appVer)
...
const exportToExcel = () => {
  let html = `
    ...
    <head>
      <meta charset="utf-8" />
      <style>
        table { border-collapse: collapse; text-align: center; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; }
        th { border: 0.5pt solid #cbd5e1; background-color: #f1f5f9; font-weight: bold; font-size: 10pt; padding: 6px; color: #334155; }
        td { border: 0.5pt solid #e2e8f0; font-size: 10pt; padding: 6px; color: #475569; }
        .site-header { background-color: #e2e8f0; color: #1e293b; }
        .total-cell { background-color: #f0fdf4; font-weight: bold; color: #166534; border: 0.5pt solid #bbf7d0; }
        .grand-total { background-color: #f8fafc; font-weight: bold; color: #0f172a; border-top: 1pt double #94a3b8; }
      </style>
    </head>
    <body>
      <table>
  ...
  pivotColumns.value.forEach(site => {
    html += ` <th colspan="${site.sdwts.length}" class="site-header">${site.name}</th> `;
  });
  html += ` <th rowspan="2" class="total-cell">TOTAL</th> </tr> <tr> `;
  ...
  pivotRows.value.forEach(row => {
    html += ` <tr> `;
    if (row.isFirstModel) {
      html += ` <td rowspan="${row.rowspan}" style="font-weight:bold; background-color:#f8fafc; color:#1e293b;">${row.maker}</td> `;
    }
    html += ` <td style="font-weight:bold;">${row.model}</td> `;
    ...
    html += ` <td class="total-cell">${row.total}</td> </tr> `;
  });

  html += ` </tbody> <tfoot> <tr> <td colspan="2" class="grand-total">SDWT TOTAL</td> `;
  
  pivotColumns.value.forEach(site => {
    site.sdwts.forEach(sdwt => {
      const key = `${site.name}_${sdwt}`;
      html += ` <td class="grand-total">${grandTotals.value[key] || 0}</td> `;
    });
  });
  html += ` <td class="total-cell" style="font-size:11pt; border-top: 1pt double #166534;">${grandTotals.value['_all'] || 0}</td> </tr> </tfoot> </table> </body> </html> `;
  ...
  link.setAttribute('download', `Equipment_Agent_Status_${dateStr}.xls`);
...
const exportDetailToCsv = () => {
  ...
  // 2. 바디 데이터 루프 순회 및 이스케이프 처리
  detailData.value.forEach((item, idx) => {
    const no = String(idx + 1);
    const eqpId = item.eqpId || '';
    const maker = item.__maker || '';
    const model = item.__model || '';
    const agentVersion = item.appVer || '-';
    const status = item.isOnline ? 'On' : 'Off';

    const row = [no, eqpId, maker, model, agentVersion, status];
    csvRows.push(row.map(v => `"${String(v).replace(/"/g, '""')}"`).join(','));
  });

  // 3. UTF-8 BOM(Byte Order Mark) 주입을 통한 엑셀 한글 깨짐 원천 방지
  ...
  link.setAttribute('href', url);
  link.setAttribute('download', `Detail_${safeTitle}_${dateStr}.csv`);
  link.style.visibility = 'hidden';
  document.body.appendChild(link);
  link.click();
  document.body.removeChild(link);
...
const rightDetailData = computed(() => {
  if (detailData.value.length < 10) return [];
  const half = Math.ceil(detailData.value.length / 2);
